...
#define TIMER_CTL_TAOTE 0x00000020         // GPTM TimerA Output Trigger Enable
#define TIMER_CTL_TAEN 0x00000001          // GPTM TimerA Enable
#define TIMER_IMR_TATOIM 0x00000001  // GPTM TimerA Time-Out Interrupt Mask
#define TIMER_TAILR_TAILRL_M \
  0x0000FFFF                           // GPTM TimerA Interval Load Register Low
...
void ADC_Open(int channelNum) {
  ...
  ADC1_SSCTL3_R = 0x0006;       // 14) no TS0 D0, yes IE0 END0  //-> 0x000E temp sensor <-//
...
void ADC0_Seq3_Timer2A_Init(uint8_t channelNum, uint32_t period) {
  ...
  TIMER2_TAMR_R =
      0x00000002;  // configure for periodic mode, default down-count settings
...
void (*ADC0Seq3_job)(unsigned long);
...
// Output: None
void ADC_Collect(uint32_t channelNum, uint32_t freq, void (*task)(unsigned long)) {
