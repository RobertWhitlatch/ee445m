...
typedef struct Sema4_s Sema4Type;
...
struct Sema4_s {
  ...
  ThreadList_t *blockedThreads; // sorted in descending priority order
};
...
// singly-linked list of TCBs
struct ThreadList_s {
  TCB_t *car;
  ThreadList_t *cdr;
};

// WARNING: these call malloc()/free() and are *not* thread-safe
...
int OS_AddPeriodicThread_Mitch(void (*task)(void), unsigned long period,
                         unsigned long priority);
